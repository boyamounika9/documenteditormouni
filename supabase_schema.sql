-- Drop existing tables/functions if they exist to start fresh
...
DROP FUNCTION IF EXISTS public.user_has_document_access(uuid, uuid);
...
-- Add tables to realtime publication (handles if already exists/re-adds them safely)
ALTER PUBLICATION supabase_realtime DROP TABLE IF EXISTS public.documents, public.document_collaborators;
ALTER PUBLICATION supabase_realtime ADD TABLE public.documents, public.document_collaborators;
